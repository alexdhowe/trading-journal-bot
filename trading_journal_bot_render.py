...
async def create_trade_exit_embed(trade_data):
    ...
    if trade_data[25]:  # market_price_at_exit
...
async def create_detailed_trade_embed(trade_data):
    ...
    if trade_data[24]:  # market_price_at_entry
...
def create_analytics_embed(analytics, username):
    """Create analytics embed (unchanged from original)"""
    embed = discord.Embed(
...
async def export_trades(ctx):
    ...
    if trades:
        ...
        for trade in trades:
            csv_content += f"{trade[3]},{trade[4]},{trade[5]},{trade[6]},{trade[24] or 'N/A'},{trade[7]},{trade[8]},{trade[15] or 'N/A'},{trade[25] or 'N/A'},{trade[16] or 'N/A'},{trade[19] or 'N/A'},{trade[20] or 'N/A'},{trade[22]}\n"
...
ENHANCED WITH POLYGON API FOR LIVE PRICES
"""
...
class TradeExitModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            symbol = trade_data[3]  # symbol column
...
# [Previous classes remain the same: CloseTradeSelectView, TradeManagementView, TradesPaginationView]
